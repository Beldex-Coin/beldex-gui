...
# cleaning "auto-generated" bitbeldex directory on "make distclean"
...
linux {
    ...
    # currently beldex has an issue with "static" build and linunwind-dev,
...
# translation stuff
TRANSLATIONS = $$files($$PWD/translations/beldex-core_*.ts)
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libbeldex-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
