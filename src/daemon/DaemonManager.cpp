...
{
    // prepare command line arguments and pass to beldexd
    ...
    qDebug() << "starting beldexd " + m_beldexd;
    ...
    // Start beldexd
    bool started = m_daemon->startDetached(m_beldexd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM beldexd.exe");
#else
                QProcess::execute("pkill beldexd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./beldexd status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type nettype, QString &message) const
{
    ...
    p.start(m_beldexd, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to beldexd
#ifdef Q_OS_WIN
    m_beldexd = QApplication::applicationDirPath() + "/beldexd.exe";
#elif defined(Q_OS_UNIX)
    m_beldexd = QApplication::applicationDirPath() + "/beldexd";
#endif

    if (m_beldexd.length() == 0) {
